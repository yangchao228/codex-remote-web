...
![LAN 模式下手机连接电脑上的 Codex](https://images.reai.group/images/0552991e828f60aa-lan-mode.jpg)
...
- 打开一个本地 Web 控制台
- 用电脑终端里的配对码完成手机配对。本机和 LAN 模式默认 6 位，remote quick 模式默认 12 位
- 在手机上选择工作目录
- 在手机上输入 Codex 任务
...
- 通过 `npm run remote:quick` 开启带 24 小时自动关闭的临时移动网络访问

## 效果预览

手机端就是一个轻量控制台：先和电脑完成配对，再选择工作区、提交任务、查看 Codex 输出和历史记录。

| 配对与连接 | 提交任务与查看输出 | 历史记录与状态 |
| --- | --- | --- |
| <img src="https://images.reai.group/images/dd93b3bbc3de7e84-1.jpg" alt="手机端配对和连接状态" width="260"> | <img src="https://images.reai.group/images/598cd2e03b5496bf-phone-task-output.jpg" alt="手机端提交 Codex 任务并查看输出" width="260"> | <img src="https://images.reai.group/images/a4e9becf576ddb05-3.jpg" alt="手机端查看最近任务和状态" width="260"> |

## 使用方式

### 低成本使用秘籍

如果你不想先看代码和安装说明，可以直接把这个 GitHub 项目下载到本地，在项目根目录启动 Codex，然后告诉它：

```text
帮我启动这个项目
```

Codex 会自己读取项目结构、`package.json` 和 README，完成安装依赖、构建、启动服务这些步骤。你只需要打开终端里打印的地址，输入配对码，就可以开始用手机控制电脑上的 Codex。

这个方式的使用成本很低：不用先理解代码，也不用先读完整安装文档。第一次使用时，建议仍然确认终端输出的访问地址、LAN 模式提示和 6 位配对码。
...
启动后看终端输出。localhost 模式打开本地地址，LAN 模式用手机打开终端打印的局域网地址，remote quick 模式用手机打开脚本最后打印的临时 HTTPS 地址。
...
- 真实 Codex CLI smoke
- LAN 模式 smoke
- remote quick 外部 URL smoke 和手机移动网络实测
